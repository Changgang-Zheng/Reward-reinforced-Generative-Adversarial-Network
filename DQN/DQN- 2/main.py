...
def save_Q_table(table, SINR, initial_real_reword, action, dronePos, episode, step, drone, name , collection_name, host='localhost', port=27017):
    ...
    for i in table.index:
        ...
        for j in table.columns:
            drone_dict['position: ' + i][j] = table.loc[i, j]
...
def save_data_for_training(Store_transition, count, action_adjust, reword_, observation_seq_adjust_):
    ...
    # Store_transition[count]['reword'] = action_reword.detach().numpy()
    Store_transition[count]['reword'] = np.array([reword_])
...
def grasp_data_for_training(Store_transition, count, numbers = 1):
    ...
    for dict in selected:
        ...
        if ('y' not in dir()) or ('state_' not in dir()):
            ...
            y = np.concatenate((y, Store[dict]['reword']), axis=0)
            action = Store[dict]['action']
    return  state_, y, action
    ...
    for i in range(args.episode):
        ...
        for j in range(args.step):
            for drone_No in range(args.numDrones):
                if i == 0 and j ==0:
                    allocVec, SINR, reword = models.alloc_users(userPos,dronePos,args.fc,args.dAngle,args.N0,args.BW,args.Pt,args.connectThresh)
                    observation_seq = DQN.observe(drone_No, allocVec, dronePos, userPos)
                    for k in range(args.sequence_len-1):
                        observation_seq = np.concatenate((observation_seq, DQN.observe(drone_No, allocVec, dronePos, userPos)), axis=2)
                allocVec, SINR, reword = models.alloc_users(userPos,dronePos,args.fc,args.dAngle,args.N0,args.BW,args.Pt,args.connectThresh)
                observation_seq = np.concatenate((observation_seq[: ,: ,3:30], DQN.observe(drone_No, allocVec, dronePos, userPos)), axis=2)
                ...
                if cf.use_cuda:
                    action_reword = target_network(torch.from_numpy(np.array([observation_seq_adjust])).cuda())
                    action_reword = action_reword.cpu()
                else:
                    action_reword = target_network(torch.from_numpy(np.array([observation_seq_adjust])))
                action_adjust = (torch.argmax(action_reword)).detach().numpy()
                # ================================= take actions =======================================================
                dronePos[drone_No][:2] = DQN.take_action(dronePos[drone_No][:2], args.action_space[action_adjust])
                allocVec_, SINR_, reword_ = models.alloc_users(userPos,dronePos,args.fc,args.dAngle,args.N0,args.BW,args.Pt,args.connectThresh)
                observation_seq_ = np.concatenate((observation_seq[: ,: ,3:30], DQN.observe(drone_No, allocVec_, dronePos, userPos)), axis=2)
                observation_seq_adjust_ = (np.swapaxes(np.swapaxes(observation_seq_,0,2),1,2)).astype(np.float32)
                # action_reword[0][action_adjust] = reword_
                Store_transition = save_data_for_training(Store_transition, count, action_adjust, reword_, observation_seq_adjust_)

                count += 1
                if count % 200 == 0 :
                    for round in range(100):
                        state_, y, action = grasp_data_for_training(Store_transition, count)
                        state_ = torch.from_numpy(state_)
                        y = torch.from_numpy(y)
                        Q_target = eval_network(state_.cuda())
                        # loss = pred_loss(y + args.LAMBDA * Q_target,  Q_target)
                        loss = DQN.pred_loss(y.cuda(), Q_target, action)
        ...
        print('All eisodes rewords:', counts)
        np.save('Rewords\\reword_episod_' + str(i) + '.npy', count)
