...
def alloc_users(userPos, dronePos, fc, dAngle, N0, BW, Pt, connectThresh):
    ...
    allocVec = np.zeros((numUsers,numDrones))
    ...
    for i in range(allocVec.shape[0]):
        for j in range(allocVec.shape[1]):
            if allocVec[i, j] > 0:
                reword += 1
                break
...
class Deep_Q_Network:
    ...
    def take_action(self, state, action):
        ...
        return state
    def pred_loss(self, reword, predict_array, a):
        predict_loss = nn.MSELoss(reduction='mean')
        loss = predict_loss(reword[0], predict_array[0, a])
        return loss
