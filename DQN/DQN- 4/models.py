...
class Deep_Q_Network:
    ...
    def observe(self, drone_No, allocVec, Drone_pos, User_pos):
        ...
        for i in range(self.args.numUsers):
                ...
                #     state[int(User_pos[i,0])-5:int(User_pos[i,0])+4,int(User_pos[i,1]), 0] = 60

                # if allocVec[i,j] == 1 and j != drone_No:
                #     state[int(User_pos[i,0])-4:int(User_pos[i,0])+3,int(User_pos[i,1]), 0] = -60
            state[int(User_pos[i, 0]), int(User_pos[i, 1]), 0] = 30


        for i in range(self.args.numDrones):
            if i == drone_No:
                state[int(Drone_pos[i,0])-5:int(Drone_pos[i,0])+4,int(Drone_pos[i,1]), 0] += 100

            else:
                state[int(Drone_pos[i,0])-4:int(Drone_pos[i,0])+3,int(Drone_pos[i,1]), 0] += -100


        return state

    def observe1(self, drone_No, allocVec, Drone_pos, User_pos):
        ...
        for j in range(self.args.numDrones):
            for i in range(self.args.numUsers):
                ...
                if allocVec[i,j] == 1 and j != drone_No:
                    state[int(User_pos[i,0])-5:int(User_pos[i,0])+4,int(User_pos[i,1]), 1] = -60
                state[int(User_pos[i, 0]), int(User_pos[i, 1]), 2] = 30
        ...
        for i in range(self.args.numDrones):
            if i == drone_No:
                ...
                state[int(Drone_pos[i][0]), int(Drone_pos[i][1]), 1] = -100
    ...
    def pred_loss(self, reward, Q_next, Q_eval, action):
        ...
        Q_target[0, action] = Q_target[0, action]+self.args.ALPHA*( reward[0] + self.args.LAMBDA * torch.max(Q_next)-Q_target[0, action])
        #Q_target[0, action] = reward[0] + 0.00001
        predict_loss = nn.MSELoss(reduction='mean')
        loss = predict_loss(Q_target[0, action] , Q_eval[0, action] )
        return loss
...
class net(nn.Module):
    def __init__(self,args):
        ...
        self.args = args
        # Encoder layers
        self.enc_conv1_1 = nn.Conv2d(1 , 64, kernel_size=3, stride=1, padding=1, bias=False)
...
class AlexNet(nn.Module):  # 定义网络，推荐使用Sequential，结构清晰
    def __init__(self):
        ...
        self.conv2 = torch.nn.Sequential(  # input_size = 27*27*96
            ...
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(3, 1)  # output_size = 13*13*256
        )
        ...
        self.conv5 = torch.nn.Sequential(  # input_size = 13*13*384
            ...
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(3, 1)  # output_size = 6*6*256
        )

        # 网络前向传播过程
        self.dense = torch.nn.Sequential(
            torch.nn.Linear(12544, 256),
            torch.nn.ReLU(),
