...
def drone_action(drone_p, center_p):
    if abs(drone_p[0]-center_p[0])>abs(drone_p[1]-center_p[1]):
        if abs(drone_p[0]-center_p[0])<10:
            action = 'stay'
        elif drone_p[0]>center_p[0]:
            action = 'west'
        else:
            action = 'east'
    else:
        if abs(drone_p[1]-center_p[1])<10:
            action = 'stay'
        elif drone_p[1]>center_p[1]:
            action = 'south'
        else:
            action = 'north'
    return action
