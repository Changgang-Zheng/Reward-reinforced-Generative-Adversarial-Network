...
parser.add_argument('--step', default=250, type=int, help='The number of steps for any turn of runs')
...
def main(args):
    ...
    dronePos, userPos, _ = environment_setup()
    ...
    for i in range(args.episode):
        ...
        for j in range(args.step):
            ...
            for k in range(args.numDrones):
                centroids, _ = models.kmeans(userPos[:,:2].tolist(), args.numDrones)
                if np.shape(np.array(centroids)) == (2,2):
                    centroid = centroids
                if np.shape(np.array(centroids)) != (2,2):
                    print('....wow....')
                    centroids = centroid
                action = models.drone_action(dronePos[k,:2], np.array(centroids[k]))
                dronePos[k][:2] = Q.take_action(dronePos[k][:2], action)
